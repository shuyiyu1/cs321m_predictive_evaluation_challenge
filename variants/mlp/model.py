...
def _signed_hash(feature: str, dim: int) -> tuple[int, float]:
    ...
    return raw % dim, (1.0 if ((raw >> 63) & 1) == 0 else -1.0)
...
def _mlp_vector(input: dict) -> dict[int, float]:
    dim = int(_MLP.get("dim", 0))
    if not dim:
        return {}
    subject = _norm(input.get("subject_content", ""))
    benchmark = _norm(input.get("benchmark", ""))
    condition = _norm(input.get("condition", "")) or "none"
    item_toks = _tokens(input.get("item_content", ""))[:450]
    subject_toks = _tokens(subject)[:100]
    vec = {}

    def add(name: str, value: float = 1.0) -> None:
        idx, sign = _signed_hash(name, dim)
        vec[idx] = vec.get(idx, 0.0) + sign * value

    add("bias")
    add(f"subject={subject}", 1.5)
    add(f"benchmark={benchmark}", 1.4)
    add(f"condition={condition}", 1.0)
    add(f"bench_condition={benchmark}|{condition}", 1.2)
    ...
    for a, b in zip(item_toks, item_toks[1:]):
        add(f"ib:{a}_{b}", 0.45)
    scale = math.sqrt(sum(v * v for v in vec.values())) or 1.0
    return {i: v / scale for i, v in vec.items()}


def _mlp_logit(input: dict) -> float:
    if not _MLP:
        return 0.0
    w1 = _MLP.get("w1", [])
    b1 = _MLP.get("b1", [])
    w2 = _MLP.get("w2", [])
    if not w1 or not b1 or not w2:
        return 0.0
    hidden = int(_MLP.get("hidden", len(b1)))
    activations = [float(b1[j]) for j in range(hidden)]
    for i, value in _mlp_vector(input).items():
        if i >= len(w1):
            continue
        row = w1[i]
        for j in range(hidden):
            activations[j] += value * float(row[j])
    out = float(_MLP.get("b2", 0.0))
